...
function renderUI(users) {
    ...
    allUsers.filter(u => u.full_name.toLowerCase().includes(term)).forEach(user => {
        ...
        card.style.cssText = `border-left:4px solid ${color}; background:rgba(255,255,255,0.05); margin-bottom:8px; padding:15px; border-radius:8px; cursor:pointer; color: white;`;
        card.innerHTML = `<b>${user.full_name}</b><br><small style="color:#64748b">${user.serial_number || 'ACTIVE'}</small>`;
...
async function init() {
    ...
    // BACK TO ORIGINAL LEAFLET TILES (OpenStreetMap)
